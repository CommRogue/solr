  ...
  public Query getQuery() throws SyntaxError {
    if (query == null) {
      ...
      if (localParams != null) {
        ...
        String name = localParams.get(NamedQueries.NAME);
        if (name != null && !name.isBlank() && query != null) {
          // record the pre-wrap query for downstream consumers that need the named queries
          // themselves rather than their matches (e.g. hl.matchedQueries)
          NamedQueries.register(req, name, query);
