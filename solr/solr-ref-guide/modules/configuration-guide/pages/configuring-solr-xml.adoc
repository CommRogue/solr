...
|===
+
Enables a node-wide, segment-level Lucene query cache shared by all cores.
It must be paired with a non-zero <<segmentQueryCacheMaxRam,`segmentQueryCacheMaxRam`>>: setting this to `true` without a size leaves the cache disabled and logs a warning.
+
...
+
Lucene decides what is worth caching, and deliberately caches far less than the `filterCache` does.
Two of its rules matter when judging whether this cache is helping:
+
* Only *costly* filters are cached — range, prefix/wildcard, and other multi-term queries.
Plain single-term filters (e.g., `fq=field:value`) are never cached, on the grounds that they are already fast.
* Only segments of at least 10,000 documents are cached, so small cores may never populate it.
+
In default configurations this property uses the value of the `solr.segmentQueryCache.enabled` system property, e.g., `-Dsolr.segmentQueryCache.enabled=true`.

[#segmentQueryCacheMaxRam]
`segmentQueryCacheMaxRam`::
+
[%autowidth,frame=none]
|===
...
|===
+
The maximum amount of heap the node-wide segment query cache may use.
...
An empty value or `0` (the default) leaves the cache disabled, even when <<enableSegmentQueryCache,`enableSegmentQueryCache`>> is `true`.
+
In default configurations this property uses the value of the `solr.segmentQueryCache.maxRam` system property, e.g., `-Dsolr.segmentQueryCache.maxRam=10%`.
...
|===
+
The maximum number of queries held by the node-wide segment query cache.
Only used when <<enableSegmentQueryCache,`enableSegmentQueryCache`>> and <<segmentQueryCacheMaxRam,`segmentQueryCacheMaxRam`>> enable the cache.
In default configurations this property uses the value of the `solr.segmentQueryCache.count` system property.
